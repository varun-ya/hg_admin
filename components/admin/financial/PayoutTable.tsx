...
import {
  ...
  Eye, HandPalm, Lightning, X, Command,
} from "@phosphor-icons/react";
import { payouts } from "./financialMockData";
import type { Payout, PayoutStatus } from "./financialTypes";
...
interface Props { onSelect: (p: Payout) => void }

function PayoutTable({ onSelect }: Props) {
  ...
  const filtered = useMemo(() => {
    ...
    return payouts.filter((p) => {
      if (q && !p.teacherName.toLowerCase().includes(q) && !p.aegisId.toLowerCase().includes(q) && !p.bankAccount.toLowerCase().includes(q)) return false;
    ...
    <div className="bg-white rounded-2xl border border-[#F0F0F0] overflow-hidden">
      <div className="px-7 pt-6 pb-5">
        <div className="flex items-center justify-between mb-1">
          <div>
            <h3 className="text-[15px] font-medium text-[#1A1A1A] font-season">Payout Queue</h3>
            <p className="text-[12px] text-[#CACACA] mt-1">{filtered.length} payout{filtered.length !== 1 ? "s" : ""}{activeFilterCount > 0 ? " (filtered)" : ""}</p>
          </div>
          <div className="flex items-center gap-3">
            <div className="relative w-[280px]">
              <MagnifyingGlass size={14} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[#CACACA]" />
              <input ref={searchRef} type="text" value={search} onChange={(e) => { setSearch(e.target.value); setPage(0); }} placeholder="Search teachers…" className="w-full h-[38px] pl-9 pr-16 border border-[#F0F0F0] rounded-xl text-[13px] text-[#1A1A1A] placeholder:text-[#DCDCDC] focus:outline-none focus:border-[#DCDCDC] focus:shadow-[0_0_0_3px_rgba(0,0,0,0.02)] transition-all font-matter bg-white" />
      ...
      {showFilters && (
        <div className="px-7 pb-5 flex items-center gap-3 flex-wrap border-b border-[#F5F5F5]">
          <FilterSelect value={statusFilter} options={[{ value: "all", label: "All Status" }, { value: "ready", label: "Ready" }, { value: "locked", label: "Locked" }, { value: "processing", label: "Processing" }, { value: "failed", label: "Failed" }, { value: "completed", label: "Completed" }]} onChange={(v) => { setStatusFilter(v as PayoutStatus | "all"); setPage(0); }} />
          {activeFilterCount > 0 && <button onClick={() => { setStatusFilter("all"); setPage(0); }} className="flex items-center gap-1 text-[11px] font-medium text-[#ACACAC] hover:text-[#1A1A1A] bg-transparent border-none cursor-pointer ml-1 transition-colors"><X size={10} weight="bold" /> Reset</button>}
        </div>
      )}
        ...
        <table className="w-full text-left border-collapse min-w-[960px]">
          <thead>
            <tr className="border-t border-b border-[#F5F5F5]">
              <th className="py-3.5 pl-7 pr-3 text-[10px] font-medium text-[#CACACA] uppercase tracking-[0.1em]">Teacher</th>
              <th className="py-3.5 px-3 text-[10px] font-medium text-[#CACACA] uppercase tracking-[0.1em]">Aegis ID</th>
              <th className="py-3.5 px-3 text-[10px] font-medium text-[#CACACA] uppercase tracking-[0.1em]">Cleared Amount</th>
              <th className="py-3.5 px-3 text-[10px] font-medium text-[#CACACA] uppercase tracking-[0.1em]">Bank / Route</th>
              <th className="py-3.5 px-3 text-[10px] font-medium text-[#CACACA] uppercase tracking-[0.1em]">Settlement Date</th>
          ...
          <tbody>
            {paged.length === 0 ? (
              <tr><td colSpan={7} className="py-20 text-center"><div className="flex flex-col items-center gap-3"><div className="w-12 h-12 rounded-2xl bg-[#F7F7F7] flex items-center justify-center"><MagnifyingGlass size={20} className="text-[#DCDCDC]" /></div><p className="text-[13px] text-[#ACACAC]">No payouts match</p></div></td></tr>
            ) : paged.map((p, i) => (
              <tr key={p.id} onClick={() => onSelect(p)} className={`hover:bg-[#FAFAFA]/60 transition-colors duration-100 cursor-pointer group ${i > 0 ? "border-t border-[#F8F8F8]" : ""}`}>
                <td className="py-4 pl-7 pr-3">
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full bg-[#F7F7F7] flex items-center justify-center text-[11px] font-medium text-[#999] shrink-0">{p.teacherName.split(" ").map(n => n[0]).join("")}</div>
                    <span className="text-[13px] text-[#1A1A1A]">{p.teacherName}</span>
                  </div>
                </td>
                <td className="py-4 px-3"><span className="text-[12px] font-mono text-[#ACACAC]">{p.aegisId}</span></td>
                <td className="py-4 px-3"><span className="text-[13px] font-normal text-[#1A1A1A] tabular-nums">${p.clearedAmount.toLocaleString()}</span></td>
                <td className="py-4 px-3"><span className="text-[12px] text-[#777]">{p.bankAccount}</span></td>
                <td className="py-4 px-3"><span className="text-[12px] text-[#777]">{p.expectedDate}</span></td>
                <td className="py-4 px-3"><span className={`inline-flex px-2.5 py-[3px] rounded-full text-[10.5px] font-medium capitalize ${STATUS_STYLE[p.status]}`}>{p.status}</span></td>
                <td className="py-4 pr-7 pl-3 text-right relative">
                  <button onClick={(e) => { e.stopPropagation(); setContextMenu(contextMenu === p.id ? null : p.id); }} className="p-1.5 rounded-lg hover:bg-[#F5F5F5] bg-transparent border-none cursor-pointer text-[#CACACA] hover:text-[#999] transition-colors opacity-0 group-hover:opacity-100"><DotsThree size={18} weight="bold" /></button>
                  {contextMenu === p.id && (
                    <div className="absolute right-7 top-full mt-1 bg-white border border-[#F0F0F0] rounded-xl shadow-[0_12px_40px_-12px_rgba(0,0,0,0.12)] py-1 z-20 min-w-[220px] animate-contextIn" onClick={(e) => e.stopPropagation()}>
                      <CtxItem icon={<Eye size={13} />} label="View Payout Details" onClick={() => { setContextMenu(null); onSelect(p); }} />
                      <CtxItem icon={<HandPalm size={13} />} label="Hold Funds" onClick={() => setContextMenu(null)} />
                      <CtxItem icon={<Lightning size={13} />} label="Manual Payout" onClick={() => setContextMenu(null)} />
                    </div>
                  )}
                </td>
              </tr>
...
export default memo(PayoutTable);
