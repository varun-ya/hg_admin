      ...
      <div className="bg-white rounded-2xl border border-[#F0F0F0] p-7 h-full flex flex-col">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-[11px] text-[#CACACA] mt-0.5">Oct 2025 – Mar 2026</p>
          </div>
          <button
        ...
        <div className="grid grid-cols-3 gap-4 mb-6">
          <div className="bg-[#FAFAFA] rounded-xl px-4 py-3">
            ...
            <p className="text-[20px] font-normal text-[#1A1A1A] tracking-tight leading-none">{totalRevenue}</p>
            {growth && (
              <span className="flex items-center gap-1 text-[10px] text-[#22C55E] mt-1.5">
          ...
          <div className="bg-[#FAFAFA] rounded-xl px-4 py-3">
            ...
            <p className="text-[20px] font-normal text-[#1A1A1A] tracking-tight leading-none">{pendingEscrow}</p>
        ...
        <div className="flex-1 min-h-0" style={{ minHeight: 160 }}>
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              <XAxis
                dataKey="date"
                tick={{ fontSize: 11, fill: "#CACACA", fontFamily: "Matter, sans-serif" }}
                axisLine={false}
                tickLine={false}
              />
              <YAxis
                tick={{ fontSize: 10, fill: "#CACACA", fontFamily: "Matter, sans-serif" }}
                axisLine={false}
                tickLine={false}
                tickFormatter={fmt}
                width={48}
              ...
              >
                {dailyData.map((_, index) => (
                  <Cell
                    key={index}
                    fill={activeIndex === index ? "url(#finBarGradHover)" : "url(#finBarGrad)"}
                  />
                ))}
              </Bar>
              <Line
                ...
                stroke="#E08A3C"
                strokeWidth={2}
                dot={{ r: 3, fill: "#E08A3C", strokeWidth: 0 }}
                activeDot={{ r: 5, fill: "#E08A3C", stroke: "#fff", strokeWidth: 2 }}
